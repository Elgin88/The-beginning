...
namespace JBooth.MicroVerseCore
{
    public class SplinePath : Stamp, IHeightModifier, ITextureModifier
    {
        ...
        float ComputeMaxSDF()
        {
            ...
            if (modifySplatMap)
                val = Mathf.Max(val, splatWeight + splatSmoothness);
            if (clearTrees)
        ...
        public override void OnEnable()
        {
            ...
            MicroVerse.instance?.Invalidate(GetBounds(), MicroVerse.InvalidateType.All);
        ...
        SplineRenderer GetSplineRenderer(Terrain terrain)
        {
            if (splineRenderers.ContainsKey(terrain))
            {
                ...
                if (sr.lastMaxSDF < mx)
                {
                    ...
                    {
                        sr.Render(spline, terrain, splineWidths, splineWidthEasing, (int)sdfRes, mx);
                    }
                }
                return sr;
            }
            else
            {
                ...
                if (terrainBounds.Intersects(GetBounds()))
                {
                    ...
                    {
                        sr.Render(spline, terrain, splineWidths, splineWidthEasing, (int)sdfRes, ComputeMaxSDF());
                    }
