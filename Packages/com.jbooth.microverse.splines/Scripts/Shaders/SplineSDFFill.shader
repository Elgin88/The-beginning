...
Shader "Hidden/MicroVerse/SplineSDFFill"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            {
                float xmin = min(min(min(p0.x, p1.x), p2.x), p3.x) - e;
                float xmax = max(max(max(p0.x, p1.x), p2.x), p3.x) + e;
                float ymin = min(min(min(p0.y, p1.y), p2.y), p3.y) - e;
                float ymax = max(max(max(p0.y, p1.y), p2.y), p3.y) + e;
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                bool area = (_Info.y > 0 && _IsArea > 0.5);
                ...
                float maxSDF = _MaxSDF * 1.5 + width;
