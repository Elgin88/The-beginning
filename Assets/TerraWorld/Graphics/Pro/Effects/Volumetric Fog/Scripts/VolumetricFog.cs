...
namespace TerraUnity.Runtime
{
	...
	public class VolumetricFog : MonoBehaviour
	{
		...
		private static VolumetricFog Script { get => GetScript(); }
		private static VolumetricFog _script;

		private static VolumetricFog GetScript()
		{
			if (_script == null)
			{
				VolumetricFog script = TCameraManager.MainCamera.GetComponent<VolumetricFog>();

				if (script != null)
					_script = script;
				else
					_script = TCameraManager.MainCamera.gameObject.AddComponent<VolumetricFog>();
			}
			//else
		...
		// Density
		public float m_ConstantFog = 0.666f;
		...
		public float m_NoiseFogScale = 0.033f;
		...
		public Color m_AmbientLightColor = new Color(0.633f, 0.766f, 0.815f, 1f);
		public Color userColor = new Color(0.633f, 0.766f, 0.815f, 1f);
		...
		public float windSpeedMultiplier = 1;

		private static VolumetricFogParams _parameters;

		public static VolumetricFogParams GetParams()
		{
			_parameters.hasVolumetricFog = Enabled;
			_parameters.m_GlobalDensityMult = Script.m_GlobalDensityMult;
			_parameters.autoColor = Script.autoColor;
			_parameters.m_NearClip = Script.m_NearClip;
			_parameters.m_FarClipMax = Script.m_FarClipMax;
			_parameters.m_ConstantFog = Script.m_ConstantFog;
			_parameters.m_NoiseFogAmount = Script.m_NoiseFogAmount;
			_parameters.m_NoiseFogScale = Script.m_NoiseFogScale;
			_parameters.m_AmbientLightColor = Script.m_AmbientLightColor;
			_parameters.userColor = Script.userColor;
			_parameters.windSpeedMultiplier = Script.windSpeedMultiplier;

			return _parameters;
		}

		public static void SetParams(VolumetricFogParams parameters, bool updateAtmosphere = true)
		{
			 _parameters = parameters;

			Enabled = parameters.hasVolumetricFog;
			Script.m_GlobalDensityMult = parameters.m_GlobalDensityMult;
			Script.autoColor = parameters.autoColor;
			Script.m_NearClip = parameters.m_NearClip;
			Script.m_FarClipMax = parameters.m_FarClipMax;
			Script.m_ConstantFog = parameters.m_ConstantFog;
			Script.m_NoiseFogAmount = parameters.m_NoiseFogAmount;
			Script.m_NoiseFogScale = parameters.m_NoiseFogScale;
			Script.m_AmbientLightColor = parameters.m_AmbientLightColor;
			Script.userColor = parameters.userColor;
		...
		void SetUpForScatter(int kernel)
		{
            ...
            {
				float m_Speed = WindManager.GetParams().windMain;
				m_InjectLightingAndDensity.SetFloat("_WindSpeed", m_Speed);

				if (WindManager.GetParams().hasWind)
				{
					Vector3 windDir = -WindManager.windDirectionForward * windSpeedMultiplier;
            ...
            {
				m_InjectLightingAndDensity.SetFloat("_WindSpeed", 0);
				m_windDir[0] = 0;
				m_windDir[1] = 0;
				m_windDir[2] = 0;
				m_InjectLightingAndDensity.SetFloats("_WindDir", m_windDir);
			...
			if (!autoColor)
				ambient = userColor * m_AmbientLightIntensity * 0.1f;
