...
void SplatmapVert(inout appdata_full v, out Input data)
{
	...
	// Sample codes to displace terrain vertices based on a procedural noise on GPU
	//float3 worldNormal = mul(unity_ObjectToWorld, float4(v.normal, 0.0)).xyz;
	//half slope = dot(worldNormal, normalize(half3(0, 1, 0)));
	//
	//if (slope >= _Angle)
	//{
	//	float3 worldPos = mul(unity_ObjectToWorld, v.vertex);
	//	float noise = SimplexNoise(v.texcoord, _NoiseScale);
	//	v.vertex.y += noise * _Displacement;
	//}

	//splatUV = (data.tc.xy * (_Control_TexelSize.zw - 1.0f) + 0.5f) * _Control_TexelSize.xy;
	//float2 uv0 = splatUV * _Splat0_ST.xy;
	//half4 n0 = tex2Dlod(_Normal0, float4(uv0, 0, 0));
	//half scale = 3;
	//v.vertex.y += n0.y * scale;

//#ifdef _PROCEDURALPUDDLES
//	float3 worldPos = mul(unity_ObjectToWorld, v.vertex);
//	half slope = dot(mul(unity_ObjectToWorld, float4(v.normal, 0.0)).xyz, normalize(half3(0, 1, 0)));
//	float noise = SimplexNoise(v.texcoord, _NoiseTiling);
//	half puddle = slope * noise;
//	half scale = 3;
//	v.vertex.y += puddle * scale;
//#endif

	//data.worldNormal = mul(unity_ObjectToWorld, float4(v.normal, 0.0)).xyz;
...
void SplatmapMix(Input IN, out half4 splat_control, out half weight, out fixed4 mixedDiffuse, inout fixed3 mixedNormal, out half _Smoothness, out half _Metallic, out half _AO)
{
	...
	//noise = SimplexNoise(float3(proceduralNoiseScale, proceduralNoiseScale, proceduralNoiseScale) * normalize(IN.worldPos)) * 0.5;
...
//#endif
	...
	//TODO: Check if the following line is needed to correct ranges
	//slope = slope * 0.5 - 0.5;

	//half fade = (slope - 0.85) / (1 - 0.85);
	//fixed4 targetColor = fixed4(0, 0.5, 0, 1);
