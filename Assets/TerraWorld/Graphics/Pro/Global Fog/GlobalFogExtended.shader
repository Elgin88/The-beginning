Shader "TerraUnity/Global Fog"
{
	...
	half4 ComputeFog (v2f i, bool distance, bool height, bool Scattering) : SV_Target
	{
		...
		if (distance) g += ComputeDistance (wsDir, dpth);
		if (height) g += ComputeHalfSpace (wsDir);
		half falloffAmount = 0.75; // 1 is default and for example 0.2 is 20% - This is to have more effect falloff in larger distances
		half fogFac = ComputeFogFactor(max(0.0, g * falloffAmount)); // Compute fog amount
		if (dpth == _DistanceParams.y) fogFac = 1.0; // Do not fog skybox
		//return fogFac; // for debugging
		float4 fogCol = unity_FogColor; // Do we need the fog color from lighting settings?
		//float4 fogCol = sunEGround;

		//if(Scattering) fogCol = sunEGround;
