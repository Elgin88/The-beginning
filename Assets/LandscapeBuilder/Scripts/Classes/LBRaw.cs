...
namespace LandscapeBuilder
{
    ...
    public class LBRaw
    {
        ...
        /// <param name="centreSize"></param>
        /// <param name="fillCorners"></param>
        /// <returns></returns>
        public Mesh CreatePreviewMesh(Vector3 vertOffset, int maxMeshResolution = 129, bool useBlending = false, float centreSize = 0f, float fillCorners = 0f)
        {
            ...
            else
            {
                ...
                int vertCount = 0;
                ushort heightUShort;
                float xDistToCentre, yDistToCentre, distToCentre, xPos, yPos;
                float blendCentreSizeSqr = centreSize * centreSize;
                float rawSourceWidthMinus1f = rawSourceWidth - 1;
                float rawSourceLengthMinus1f = rawSourceLength - 1;
                ...
                for (int x = 0; x < rawSourceWidth; x += pixelScalingX)
                {
                    for (int z = 0; z < rawSourceLength; z += pixelScalingZ)
                    {
                        ...
                        float rawHeightN = LBLandscapeTerrain.Normalise(heightUShort, rawMinHeight, rawMaxHeight);

                        xPos = (float)x / rawSourceWidthMinus1f;
                        yPos = (float)z / rawSourceLengthMinus1f;

                        if (useBlending)
                        {
                            // Approximate the blending of the modifier (does not take into consideration the existing terrain data)
                            xDistToCentre = (xPos - 0.5f) * 2f;
                            yDistToCentre = (yPos - 0.5f) * 2f;
                            distToCentre = (Mathf.Pow(xDistToCentre > 0f ? xDistToCentre : -xDistToCentre, (fillCorners * 2f) + 2f) +
                                Mathf.Pow(yDistToCentre > 0f ? yDistToCentre : -yDistToCentre, (fillCorners * 2f) + 2f) -
                            ...
                            rawHeightN *= 1f - Mathf.Clamp01(distToCentre);
                        }
                        ...
                        //vertPosition = new Vector3((float)x / (float)(rawSourceWidth - 1), rawHeightN, (float)z / (float)(rawSourceLength - 1));
                        vertPosition = new Vector3(xPos, rawHeightN, yPos);
