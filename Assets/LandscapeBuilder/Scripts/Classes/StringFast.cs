...
namespace LB_Extension
{
    ...
    /// Includes SCSM Extensions added by SCSM Pty Ltd.
    ///</summary>
    public class StringFast
    {
        ...
        #region Value controls methods: use a value to check if the string has to be regenerated.
        ...
        #region Append methods, to build the string without allocation
        ...
        {
            StringFast sf = new StringFast(6);

            if (hours == 0) { sf.Set("00"); }
            else if (hours < 10) { sf.Set("0"); sf.Append(hours); }
            else if (hours < 99) { sf.Set(hours); }
            else { sf.Set("--"); }
            sf.Append(":");
        ...
        {
            StringFast sf = new StringFast(15);
        ...
        public static string FormatTime(int hours, int minutes, int secs, int msecs, bool showTenths)
        {
            ...
            if (showTenths)
            {
                if (msecs > 99) { sf.Append("--"); }
                else if (msecs > 9) { sf.Append(msecs); }
                else { sf.Append("0"); sf.Append(msecs); }
            }
            else
            {
                if (msecs > 999) { sf.Append("---"); }
                else if (msecs > 99) { sf.Append(msecs); }
                else if (msecs > 9) { sf.Append("0"); sf.Append(msecs); }
                else { sf.Append("00"); sf.Append(msecs); }
            }
        ...
        public static string FormatTime(int minutes, int secs, int msecs, bool showTenths)
        {
            ...
            StringFast sf = new StringFast(10);

            if (minutes > 59) { sf.Set("--"); }
            else if (minutes > 9) { sf.Set(minutes); }
            else { sf.Set("0"); sf.Append(minutes); }
            sf.Append(":");
