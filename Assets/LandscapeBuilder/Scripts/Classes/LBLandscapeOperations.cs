...
namespace LandscapeBuilder
{
    public class LBLandscapeOperations
    {
        ...
        public static List<LBMesh> GetMeshDataFromLandscape(LBLandscape landscape, string meshPrefix, Texture2D outlineTex, Vector4 outlineTexBoundsN, UnityEngine.Color outlineColour)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //Vector4 terrainBoundsN = Vector4.zero;
                    ...
                    for (int t = 0; t < landscapeTerrains.Length; t++)
                    {
                        ...
                        if (useOutlineTex)
                        {
                            ...
                            //terrainBoundsN.x = terrainLandscapePos.x / landscape.size.x;
                            //terrainBoundsN.y = terrainLandscapePos.z / landscape.size.y;
                            //terrainBoundsN.z = (terrainLandscapePos.x + tData.size.x) / landscape.size.x;
                            //terrainBoundsN.w = (terrainLandscapePos.z + tData.size.z) / landscape.size.y;
        ...
        public static Transform CreateMeshList(LBLandscape landscape, List<LBMesh> lbMeshList, Transform tfmParent, string meshParentName, bool isStatic)
        {
            ...
            {
                // Find or Create a parent transform for the meshes under the trfmParent supplied.
                ...
                if (meshParentGameObject != null)
                {
                    ...
                    if (isSuccessful) { newMeshTransform = meshParentTrfm; }
                }
            }
        ...
        /// <param name="useMicroSplat"></param>
        /// <returns></returns>
        public static Transform CreateLandscapeMesh(LBLandscape landscape, List<LBMesh> lbMeshList, string meshParentName, bool addColliders, bool includeOcclusionAreas, bool isStatic, bool includeSplatmaps, bool useMegaSplat, bool saveMeshToProjectFolder, bool useMicroSplat = false)
        {
            Transform newMeshTransform = null;
            ...
            else
            {
                ...
                if (meshParentTrfm != null)
                {
                    ...
                    {
                        #if __MICROSPLAT__
                        // If MicroSplat - mesh Terrains extension asset was installed we "could"
                        // have added MicroSplatMeshTerrain script to the Meshes parent,
                        // then added each mesh renderer to the MicroSplatMeshTerrain.MeshTerrains array.
                        // We'd also have had to add the splatMaps to the ControlTextures array.
                        // HOWEVER, the MicroSplat extension comes with a conversion utility which does
                        // everything including creating the mesh terrains...
                        #if UNITY_EDITOR
                        Debug.LogWarning("LB MicroSplat integration for Mesh Terrains is currently not implemented. Use the MicroSplat Terrain to Mesh tool instead.");
                        #endif

                        #endif
                    }
                    #region MegaSplat setup material
                    else if (useMegaSplat)
                    {
#if __MEGASPLAT__
                    ...
                    if (isOkToCreateMeshes)
                    {
                        ...
                        #region Save Mesh to Project Folder
                            ...
                            if (meshTransform != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        #region Assign MegaSplat material
        ...
        /// <summary>
        /// [WIP - INCOMPLETE]
        /// Attempt to blend the splatmaps into a coloured blend that represents what the landscape
        ...
        /// <param name="showErrors"></param>
        public static void ExportSplatmapsAsBlendedImage(LBLandscape landscape, bool flipX, bool flipZ, bool showErrors)
        {
            string methodName = "LBLandscapeOperations.ExportSplatmapsAsBlendedImage";
            ...
            else
            {
                ...
                else
                {
                    ...
                    int numTerrainLayers = tData.terrainLayers == null ? 0 : tData.terrainLayers.Length;
                    ...
                    int alphamapResolution = tData.alphamapResolution;

                    // Ensure texture size is a power of 2 by using alphamapResolution.
                    int texWidth = numTerrainsWide * alphamapResolution;

                    // Clamp max texture size to 8192x8192
                    if (texWidth > 8192) { texWidth = 8192; }

                    //Debug.Log("[DEBUG] Blended image resolution: " + texWidth + " x " + texWidth);

                    int splatMapRes = texWidth * texWidth;
                    float splatTexWidthLess1F = (float)(texWidth - 1);

                    if (numAlphaMapLayers > 0)
                    {
                        List<Texture2D> splatTextureList = new List<Texture2D>(numAlphaMapLayers);
                        ...
                        for (int alphaIdx = 0; !isError && alphaIdx < numAlphaMapLayers; alphaIdx++)
                        {
                            ...
                            Texture2D splatTextureIn = new Texture2D(texWidth, texWidth, TextureFormat.ARGB32, false);

                            // Output in a "standard" RGBA format (untested)
                            Texture2D splatTextureOut = new Texture2D(texWidth, texWidth, TextureFormat.RGBA32, false);

                            if (splatTextureIn == null) { if (showErrors) { Debug.LogWarning("ERROR " + methodName + " - failed to create new splatmap Texture for alpha layer " + alphaIdx); break; } }
                            else if (splatTextureOut == null) { if (showErrors) { Debug.LogWarning("ERROR " + methodName + " - failed to create new output Texture for alpha layer " + alphaIdx); break; } }
                            else
                            {
                                splatTextureIn.name = "Splatmap" + Mathf.FloorToInt(((float)alphaIdx/4f)).ToString("0") + "Tex" + alphaIdx.ToString("00");

                                splatTextureOut.name = splatTextureIn.name;
                                ...
                                for (int terrainIdx = 0; terrainIdx < numTerrains; terrainIdx++)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            LBLandscapeTerrain.ExportTextureFromSplatmap(landscape, numTerrains, terrain, terrainRow, terrainCol, alphaIdx, splatTextureIn, "R", showErrors);
                                        }
                                    }
                                }
                                ...
                                if (!isError)
                                {
                                    ...
                                    if (numTerrainLayers > alphaIdx)
                                    {
                                        TerrainLayer terrainLayer = tData.terrainLayers[alphaIdx];

                                        if (terrainLayer != null)
                                        {
                                            Texture2D diffTex = terrainLayer.diffuseTexture;
                                            Vector2 diffTexTileSize = terrainLayer.tileSize;
                                            Vector2 spPos = Vector2.zero;

                                            if (diffTex != null)
                                            {
                                                int diffWidth = diffTex.width;
                                                int diffHeight = diffTex.height;

                                                float diffWidthMinus1F = diffWidth - 1;
                                                float diffHeightMinus1F = diffHeight - 1;

                                                Color[] diffColours = diffTex.GetPixels();
                                                Color[] splatmapColours = splatTextureIn.GetPixels();

                                                int numPixels = diffColours == null ? 0 : diffColours.Length;
                                                int splatmapSize = splatmapColours == null ? 0 : splatmapColours.Length;

                                                Color[] splatmapColoursOut = new Color[splatmapSize];

                                                // Loop through the splatmap texture and apply its weight
                                                // to the terrain ground texture to produce a coloured texture.
                                                for (int spH = 0; spH < texWidth; spH++)
                                                {                                                    
                                                    float spHN = (float)spH / splatTexWidthLess1F;

                                                    // Get the landscape-space position in metres from bottom
                                                    spPos.y = spHN * landscapeSize.y;

                                                    // Works the same as Mathf.Repeat(spPos.y, diffTextileSize.y) for +ve numbers
                                                    while (spPos.y > 0f && spPos.y > diffTexTileSize.y) { spPos.y -= diffTexTileSize.y; }

                                                    // Calc normalised position of the ground texture within the splatmap.
                                                    int cPY = Mathf.FloorToInt(spPos.y / diffTexTileSize.y * diffHeightMinus1F);

                                                    for (int spW = 0; spW < texWidth; spW++)
                                                    {
                                                        float spWN = (float)spW / splatTexWidthLess1F;

                                                        // Get the landscape-space position in meters from left
                                                        spPos.x = spWN * landscapeSize.x;

                                                        // y offset in ground texture. Get start of repeating texture
                                                        // calculating how many whole textures fit before it in the
                                                        // landscape on the y-axis.

                                                        //int numTexYBefore = Mathf.FloorToInt(spPos.y / diffTexTileSize.y);

                                                        // Works the same as Mathf.Repeat(spPos.x, diffTextileSize.x) for +ve numbers
                                                        while (spPos.x > 0f && spPos.x > diffTexTileSize.x) { spPos.x -= diffTexTileSize.x; }

                                                        Color splatWeight = splatmapColours[spH * texWidth + spW];

                                                        // Get the colour of the terrain texture from the normalised position
                                                        int cPX = Mathf.FloorToInt(spPos.x / diffTexTileSize.x * diffWidthMinus1F);
  
                                                        Color texColour = diffColours[cPY * diffWidth + cPX];

                                                        texColour.r *= splatWeight.r;
                                                        texColour.g *= splatWeight.r;
                                                        texColour.b *= splatWeight.r;
                                                        texColour.a = 1f;

                                                        if (flipX && !flipZ)
                                                        {
                                                            outputIdx = (texWidth - 1 - spH) * texWidth + spW;
                                                        }
                                                        else if (!flipX && flipZ)
                                                        {
                                                            outputIdx = spH * texWidth + (texWidth - 1 - spW);
                                                        }
                                                        else if (flipX && flipZ)
                                                        {
                                                            // To flip x and z from the default behaviour is to use the original
                                                            // splatmap orientation
                                                            outputIdx = spH * texWidth + spW;
                                                        }
                                                        else
                                                        {
                                                            // Flip the x and y-axis to produce a regular image with the same
                                                            // orientation as the terrain, if placed on a plane.
                                                            outputIdx = (texWidth - 1 - spH) * texWidth + (texWidth - 1 - spW);
                                                        }

                                                        // Output the weighted colour from the texture
                                                        splatmapColoursOut[outputIdx] = texColour;
                                                    }
                                                }

                                                splatTextureOut.SetPixels(splatmapColoursOut);
                                                splatTextureOut.Apply();
                                            }
                                        }
                                    }
                                    else
                                    {
                                        if (isError) { if (showErrors) { Debug.LogWarning("ERROR " + methodName + " - could not find Unity TerrainLayer " + alphaIdx); } }
                                        isError = true;
                                    }
                                }
                                ...
                                if (!isError)
                                {
                                    splatTextureList.Add(splatTextureOut);
                                }
                            }
                        }

                        // If no errors were encountered, attempt to blend all textures that were created
                        // from the splatmaps and terrain (ground) textures.
                        if (!isError && splatTextureList.Count > 0)
                        {
                            Texture2D blendedTextureOut = LBTextureOperations.CreateTexture(texWidth, texWidth, Color.clear, false, TextureFormat.RGBA32, false);
                            if (blendedTextureOut != null)
                            {
                                blendedTextureOut.name = "SplatmapBlended";
                                if (flipX) { blendedTextureOut.name += "_FlipX"; }
                                if (flipZ) { blendedTextureOut.name += "_FlipZ"; }

                                Color[] blendedColours = blendedTextureOut.GetPixels();

                                int blendedColoursSize = blendedColours == null ? 0 : blendedColours.Length;

                                int numSplatTextures = splatTextureList.Count;

                                for (int sTexIdx = 0; sTexIdx < numSplatTextures && sTexIdx < 10; sTexIdx++)
                                {
                                    Color[] splatmapColours = splatTextureList[sTexIdx].GetPixels();

                                    for (int pixelIdx = 0; pixelIdx < blendedColoursSize; pixelIdx++)
                                    {
                                        Color splatmapColour = splatmapColours[pixelIdx];
                                        Color blendedColour = blendedColours[pixelIdx];

                                        blendedColour.r += splatmapColour.r;
                                        blendedColour.g += splatmapColour.g;
                                        blendedColour.b += splatmapColour.b;
                                        blendedColour.a = 1f;

                                        blendedColours[pixelIdx] = blendedColour;
                                    }
                                }

                                blendedTextureOut.SetPixels(blendedColours);
                                blendedTextureOut.Apply();

                                string exportPNGFilePath = splatProjectFolderPath + "/" + blendedTextureOut.name + ".png";
                                LBEditorHelper.SaveMapTexture(blendedTextureOut, exportPNGFilePath, blendedTextureOut.width);
                                Debug.Log("Exported landscape blended texture as PNG to " + exportPNGFilePath + " successfully.");
