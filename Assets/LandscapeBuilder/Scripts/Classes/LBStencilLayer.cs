...
namespace LandscapeBuilder
{
    ...
    public class LBStencilLayer
    {
        ...
        /// <summary>
        /// Return a 0.0 - 1.0 value for a normalised position in the Stencil Layer.
        /// Always return 0 if input parameters are out of range.
        /// </summary>
        /// <param name="normalisedX"></param>
        /// <param name="normalisedY"></param>
        /// <returns></returns>
        public float GetValueAtPoint(float normalisedX, float normalisedY)
        {
            int layerX, layerY;

            if (normalisedX < 0f || normalisedX > 1f || normalisedY < 0f || normalisedY > 1f) { return 0f; }
            ...
            layerX = Mathf.RoundToInt(normalisedX * (_layerResolution - 1));
            layerY = Mathf.RoundToInt((1f - normalisedY) * (_layerResolution - 1));

            return (float)layerArray[layerX, layerY] / 65535f;
        }

        /// <summary>
        /// Reverse or invert the data in the layer to produce a negative effect
        /// </summary>
        public void NegativeLayer()
        {
            // Remember if the layer array was already created
            bool isAllocated = layerArray != null;

            if (!isAllocated) { AllocLayerArray(); }
            UnCompressToUShort();
            ...
            for (int x = 0; x < layerArrayDimension1; x++)
            {
                for (int y = 0; y < layerArrayDimension2; y++)
                {
                    layerArray[x, y] = (ushort)(65535 - layerArray[x, y]);
                }
            }
            ...
            // Clean up
            if (!isAllocated) { DeallocLayerArray(); }
        }
        ...
        public void CopyUShortToRenderTexture()
        {
            ...
            else
            {
                ...
                if (layerArrayDimension1 < renderTexture.width)
                {
                    ...
                    #else
                    renderTexture.Reinitialize(layerArrayDimension1, layerArrayDimension2);
                    #endif
                }
