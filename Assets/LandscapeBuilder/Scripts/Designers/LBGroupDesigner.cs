...
namespace LandscapeBuilder
{
    ...
    public class LBGroupDesigner : MonoBehaviour
    {
        ...
        [HideInInspector] public bool showGroupFlattenArea = false;
        ...
        private UnityEngine.Color groupFlattenAreaColour = new Color(255f / 255f, 0f / 255f, 0f / 255f, 0.1f);
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                if (showGroupFlattenArea && lbGroup.isTerrainFlattened)
                {
                    using (new Handles.DrawingScope(groupFlattenAreaColour))
                    {
                        Handles.DrawSolidDisc(grpBaseTrfmPosition, Vector3.up, lbGroup.maxClearingRadius - (lbGroup.maxClearingRadius * lbGroup.flattenBlendRate));
                    }
                }
        ...
        /// <summary>
        /// Select only the zone using its zero-based in position in the list of zones.
        /// Deselect all other group-level zones.
        /// </summary>
        /// <param name="zoneIndex"></param>
        public void SingleSelectZone(int zoneIndex)
        {
            int numZones = lbGroup == null || lbGroup.zoneList == null ? 0 : lbGroup.zoneList.Count;

            if (zoneSelectedList == null) { zoneSelectedList = new List<int>(numZones + 1); }

            if (zoneSelectedList != null) { zoneSelectedList.Clear(); }

            if (zoneIndex >= 0 && zoneIndex < numZones)
            {
                zoneSelectedList.Add(zoneIndex);
            }
        }

        /// <summary>
        /// Unselect all group-level zones in the designer
        /// </summary>
