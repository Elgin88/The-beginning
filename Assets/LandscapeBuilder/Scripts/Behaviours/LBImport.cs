    ...
    public class LBImport : MonoBehaviour
    {
        ...
        public static void ImportTextures(LBLandscape landscape, bool showErrors)
        {
            ...
            else
            {
                ...
                #if UNITY_EDITOR
                if (landscape.terrainTexturesList.FindIndex(txt => txt.texturingMode == LBTerrainTexture.TexturingMode.Imported) >= 0)
                {
                    isContinue = LBEditorHelper.PromptYesNo("Import Textures from Terrains", "Do you want to overwrite existing imported Texture data?");

                    if (isContinue)
                    {
                        int numGrass = landscape.terrainGrassList.Count;
                        for (index = numGrass - 1; index >= 0; index--)
                        {
                            LBTerrainTexture lbTerrainTexture = landscape.terrainTexturesList[index];
                            if (lbTerrainTexture != null && lbTerrainTexture.texturingMode == LBTerrainTexture.TexturingMode.Imported)
                            {
                                landscape.terrainTexturesList.RemoveAt(index);
                            }
                        }
                    }
                }
        ...
        public static void ImportTrees(LBLandscape landscape, bool showErrors)
        {
            ...
            else
            {
                ...
                #if UNITY_EDITOR
                if (landscape.terrainTreesList.FindIndex(tr => tr.treePlacingMode == LBTerrainTree.TreePlacingMode.Imported) >= 0)
                {
                    isContinue = LBEditorHelper.PromptYesNo("Import Trees from Terrains", "Do you want to overwrite existing imported Tree data?");

                    if (isContinue)
                    {
                        int _numTreeTypes = landscape.terrainTreesList.Count;
                        for (index = _numTreeTypes - 1; index >= 0; index--)
                        {
                            LBTerrainTree lbTerrainTree = landscape.terrainTreesList[index];
                            if (lbTerrainTree != null && lbTerrainTree.treePlacingMode == LBTerrainTree.TreePlacingMode.Imported)
                            {
                                landscape.terrainTreesList.RemoveAt(index);
                            }
                        }
                    }
                }
        ...
        public static void ImportGrass(LBLandscape landscape, bool showErrors)
        {
            ...
            else
            {
                ...
                #if UNITY_EDITOR
                if (landscape.terrainGrassList.FindIndex(gr => gr.grassPlacingMode == LBTerrainGrass.GrassPlacingMode.Imported) >= 0)
                {
                    isContinue = LBEditorHelper.PromptYesNo("Import Grass from Terrains", "Do you want to overwrite existing imported Grass data?");
                    ...
                    if (isContinue)
                    {
                        ...
                        for (index = numGrass - 1; index >= 0; index--)
                        {
                            LBTerrainGrass lbTerrainGrass = landscape.terrainGrassList[index];
                            if (lbTerrainGrass != null && lbTerrainGrass.grassPlacingMode == LBTerrainGrass.GrassPlacingMode.Imported)
                            {
                                landscape.terrainGrassList.RemoveAt(index);
                            }
                        }
                    }
                }
                    ...
                    if (terrainGrassList != null)
                    {
                        ...
                        if (terrainGrassList.Count > 0)
                        {
                            ...
                            for (int d = terrainGrassList.Count - 1; d >= 0; d--)
                            {
                                ...
                                // TODO - also consider importing mesh grass with/without useInstancing (U2021.2+)
                                if (lbTerrainGrass != null)
                                {
