Shader "Hidden/TerrainEngine/Splatmap/LBTerrain-AddPass" 
{
	Properties 
	{
	...
	SubShader 
	{
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			splatControl /= (alphaWeight + 1e-3f);
				...
				float3 realWorldNormal = WorldNormalVector(IN, float3(0, 0, 1));
					...
					//fixed3 col;
					//col = splatControl.r * tex2D (_Splat0, IN.worldPos).rgb;
					//col += splatControl.g * tex2D (_Splat1, IN.worldPos).rgb;
					//col += splatControl.b * tex2D (_Splat2, IN.worldPos).rgb;
					//col += splatControl.a * tex2D (_Splat3, IN.worldPos).rgb;
					//o.Albedo = col;
		...
		void SplatmapFinalColor(Input IN, TERRAIN_SURFACE_OUTPUT o, inout fixed4 color)
		{
			...
			// Fog needs to fade to black for AddPass shader
