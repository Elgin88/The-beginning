...
namespace LandscapeBuilder
{
    ...
    public class LBEditorIntegration : MonoBehaviour
    {
        ...
        public static bool MicroSplatCompileShader(LBLandscape landscape, Material terrainMat, bool showErrors)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // In MS 3.7+ the default shader no longer includes "MicroSplat" so also check the material name.
                    else if (!(shader.name.Contains("MicroSplat") || terrainMat.name.Contains("MicroSplat")))
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (LBIntegration.CompareVersionNumbers(GetMicroSplatVersion(), "3.5") < 0)
                                {
                                    // < version 3.5
                                    //string meshOverlayShader = microSplatCompiler.Compile(shaderFeatures, "MicroSplat/" + landscape.name, null, true);
                                    string meshOverlayShader = (string)microSplatCompiler.GetType().InvokeMember("Compile", System.Reflection.BindingFlags.InvokeMethod, null, microSplatCompiler, new object[] { shaderFeatures, "MicroSplat/" + landscape.name, null, true });
                                ...
                                {
                                    // Version 3.5+
                                    //string meshOverlayShader = microSplatCompiler.Compile(shaderFeatures, "MicroSplat/" + landscape.name, null, null);
                                    string meshOverlayShader = (string)microSplatCompiler.GetType().InvokeMember("Compile", System.Reflection.BindingFlags.InvokeMethod, null, microSplatCompiler, new object[] { shaderFeatures, "MicroSplat/" + landscape.name, null, null });
        ...
        /// <summary>
        /// Get the current version of MicroSplat installed in the project
        /// </summary>
        /// <returns></returns>
        private static string GetMicroSplatVersion()
        {
