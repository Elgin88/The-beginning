...
namespace LandscapeBuilder
{
    ...
    /// classes are automatically excluded at runtime.
    /// </summary>
    public class LBImportTIFF : MonoBehaviour
    {
        ...
        public static LBTerrainData ImportHeightmapTIFF(LBLandscape landscape, Terrain terrain, string filePath, ref ushort minHeight, ref ushort maxHeight, bool showErrors)
        {
            ...
            Debug.Log("INFO: " + methodName + " importing TIFF for terrain: " + terrain.name + " using file: " + filePath);
                ...
                try
                {
                    ...
                    {
                        #region Read File Meta-Data
                        bool isTitled = tiff.IsTiled();
                        ...
                        int tileWidth = isTitled ? tiff.GetField(TiffTag.TILEWIDTH)[0].ToInt() : 0;
                        int tileLength = isTitled ? tiff.GetField(TiffTag.TILELENGTH)[0].ToInt() : 0;
                        int tileSize = isTitled ? tiff.TileSize() : 0;
                        int numTiles = isTitled ? tiff.GetField(TiffTag.TILEBYTECOUNTS).Length : 0;
                        ...
                        #region Debug Meta-Data
                        #if LBIMPORT_TIFF_DEBUG_MODE

                        // This fails when IsTiled() is true
                        int rowsPerStrip = isTitled ? 0 : tiff.GetField(TiffTag.ROWSPERSTRIP)[0].ToInt();

                        Debug.Log("INFO: " + methodName + " isTiled? " + isTitled.ToString() +
                                   " samplesPerPixel:" + samplesPerPixel + " bitsPerSample:" + bitsPerSample + " imgWidth:" + imgWidth + " imgLength:" + imgLength +
                                  " Num Tiles: " + numTiles + (isTitled ? " tileWidth:" + tileWidth + " tileLength:" + tileLength : "") +
                                  " rowsPerStrip:" + rowsPerStrip + " imgCompression: " + Enum.GetName(typeof(Compression), imgCompression) + " [" + imgCompression + "]" +
                                  " SampleFormat: " + Enum.GetName(typeof(SampleFormat), sampleFormat) +
                                  " scanlineSize: " + scanlineSize +
                                  " Num Directories: " + tiff.NumberOfDirectories()
                                  );

                        //var cdxs = tiff.GetConfiguredCodecs();
                        //foreach (var cdx in cdxs)
                        //{
                        //    Debug.Log("[DEBUG] codec " + cdx.m_name + " scheme: " + cdx.m_scheme.ToString() + " canDecode: " + cdx.CanDecode);
                        //}

                        // For Tiled, an array of the number of (compressed) bytes in that tile
                        ...
                        //long[] tileOffsets = tiff.GetField(TiffTag.TILEOFFSETS)[0].TolongArray();
                        #else
                        // Keep compiler happy
                        if (numTiles < 0 && imgCompression < 0) { }
                        #endif
                        #endregion

                        // NOTE: Orientation seems to be obsolete and almost always returns TOPLEFT
                        // Some files don't set the ORIENTATION which will cause method to fail if trying to access it.
                        ...
                        {                           
                            int tiffImageBufferSize = 0;
                            byte[] tiffImageBuffer;
                            // For tiled we read 1 tile (chunk) at a time.
                            // For non-tiled we read 1 scanline at a time
                            int readBufferSize = isTitled ? tileSize : scanlineSize;
                            byte[] readBuffer = new byte[readBufferSize];

                            // Attempt to allocate enough space for the whole image
                            tiffImageBufferSize = imgWidth * imgLength * bytesPerSample;
                            tiffImageBuffer = new byte[tiffImageBufferSize];
                            int tiffImageOffset = 0;

                            bool isDataReadIntoImageBuffer = false;

                            if (tiffImageBuffer == null) { if (showErrors) { Debug.LogWarning("INFO: " + methodName + " could not reserve memory to hold the TIFF data."); } }
                            else if (readBuffer == null) { if (showErrors) { Debug.LogWarning("INFO: " + methodName + " could not reserve memory to scan TIFF data " + (isTitled ? "tiles" : "lines.")); } }
                            else if (isTitled)
                            {
                                #region Titled Data

                                // Tiles should be ordered left-to-right and top-to-bottom

                                int tileWidthChunks = imgWidth > 0 ? (imgWidth / tileWidth) + 1 : 0;
                                int tileLengthChunks = imgLength > 0 ? (imgLength / tileLength) + 1 : 0;

                                int numTilesWide = (int)Math.Ceiling(imgWidth / (float)tileWidth);
                                int numTilesLong = (int)Math.Ceiling(imgLength / (float)tileLength);

                                int uncompTileWidth = imgWidth / numTilesWide;
                                int uncompTileLength = imgLength / numTilesLong;

                                #if LBIMPORT_TIFF_DEBUG_MODE
                                Debug.Log("[DEBUG] LBImportTIFF tileSize:" + tileSize + " wChks:" + tileWidthChunks + " hChks:" + tileLengthChunks + " tiffImageBufferSize:" + tiffImageBufferSize);
                                #else
                                // Keep complier happy
                                if (tileWidthChunks < 0 && tileLengthChunks < 0 && uncompTileWidth < 0 && uncompTileLength < 0) { }
                                #endif

                                int bytesProcessed = 0;

                                // Works with 8bit tiled
                                for (int row = 0; row < imgLength; row += tileLength)
                                {
                                    for (int col = 0; col < imgWidth; col += tileWidth)
                                    {
                                        // Read the tile
                                        int bytesRead = tiff.ReadTile(readBuffer, 0, col, row, 0, 0);

                                        if (bytesRead > 0)
                                        {
                                            //Debug.Log("[DEBUG] Read Tile file. bytesRead:" + bytesRead);

                                            int index = 0;

                                            // Iterate the rows in the tile
                                            for (int i = 0; i < tileLength && i + row < imgLength; i++)
                                            {
                                                // The number of samples or pixels in the current row
                                                int length = tileWidth;

                                                // Offset in the destination buffer of the first byte in the row
                                                tiffImageOffset = ((row + i) * imgWidth + col) * bytesPerSample;

                                                // Check we are not outside the image
                                                if (col + length > imgWidth)
                                                {
                                                    length = imgWidth - col;
                                                }

                                                // Copy the whole row from the tile buffer to the destination Image Buffer.
                                                Buffer.BlockCopy(readBuffer, index * bytesPerSample, tiffImageBuffer, tiffImageOffset, length * bytesPerSample);
                                ...
                                isDataReadIntoImageBuffer = bytesProcessed > 0;

                                #endregion
                            }
                            else
                            {
                                #region Non-Titled Data
                                ...
                                {
                                    if (tiff.ReadScanline(readBuffer, sl))
                                    {
                                        // Copy the line into the image buffer
                                        Buffer.BlockCopy(readBuffer, 0, tiffImageBuffer, tiffImageOffset, scanlineSize);
                                ...
                                isDataReadIntoImageBuffer = (tiffImageOffset / scanlineSize) == imgLength;

                                #endregion
                            }

                            #region Import into LBTerrainData
                            if (isDataReadIntoImageBuffer)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        for (int byteIndex = 0; byteIndex < rawHeightDataLength - 1; byteIndex += 2)
                                        {
                                            ...
                                            tiffImageOffset = ((int)imgCoords.y * imgWidth * bytesPerSample) + (int)imgCoords.x * bytesPerSample;
                                            ...
                                            else if (bitsPerSample == 32)
                                            {
                                                if ((SampleFormat)sampleFormat == SampleFormat.IEEEFP)
                                                {
                                                    ...
                                                    // The float data is heights in metres, rather than 0-1 as expected.
        ...
        /// Optionally pass in an upper clip threshold value.
        /// </summary>
        /// <param name="landscape"></param>
        /// <param name="lbLayer"></param>
        /// <param name="showErrors"></param>
        /// <param name="threshold"></param>
        /// <param name="pixelRange"></param>
        public static void FixGeoTIFFHeightmap(LBLandscape landscape, LBLayer lbLayer, bool showErrors, float threshold = 65000f, int pixelRange = 6)
        {
            ...
            else if (lbLayer.lbTerrainDataList.Count < 1) { if (showErrors) { Debug.LogWarning("ERROR: " + methodName + " - lbLayer.lbTerrainDataList does not contain any data. Please report"); } }
            {
                ...
                ushort upperClip = (ushort)threshold;
                ...
                // Set the range and let maxPixels be +1 for coding convenience
                // This the number of pixels to be checked around the pixel being tested.
                int minPixel = -pixelRange;
                int maxPixel = pixelRange + 1;

                ushort sampleMinValue, sampleMaxValue;
                // Multiple passes have little or no effect
                int numPasses = 1;
                ...
                for (int lbTDataIdx = 0; lbTDataIdx < numTerrainData; lbTDataIdx++)
                {
                    ...
                    if (lbTerrainData != null && lbTerrainData.HasRAWHeightData())
                    {
                        ...
                        {
                            sampleMinValue = ushort.MaxValue;
                            sampleMaxValue = ushort.MinValue;
                                ...
                                for (int x = 0; x < heightmapResolution; x++)
                                {
                                    ...
                                    if (lbLayer.isBelowSeaLevelDataIncluded)
                                    {
                                        ...
                                        if ((z == 0) && heightUShort == 0)
                                        {
                                            ...
                                            for (int pz = minPixel; pz < maxPixel; pz++)
                                            {
                                                ...
                                                for (int px = minPixel; px < maxPixel; px++)
                                                {
